...
describe UsersController, type: :controller do
  ...
  describe "POST #update" do
    before do
      @user = create(:user, accepted_terms: false, secure_password: false)
      @request.session[:user_id] = @user.id
    ...
    it "properly updates usser attributes" do
      ...
      expect(@user.secure_password).not_to be # Updating user information won't influence their password state.
      expect(flash[:success]).to be_present
    ...
    it "renders #edit on unsuccessful save" do
      ...
      expect(@user.secure_password).not_to be # Not updating user information won't influence their password state.
      expect(response).to render_template(:edit)
    end
  ...
  describe "POST #update_password" do
    ...
    context "with 'terms and conditions' exist and without acceptance." do
      ...
      it "properly updates users password" do
        ...
        expect(@user.secure_password).to be
        expect(flash[:success]).to be_present
      ...
      it "doesn't update the users password if initial password is incorrect" do
        ...
        expect(@user.secure_password).not_to be

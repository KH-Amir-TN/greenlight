...
class UsersController < ApplicationController
  ...
  def update
    ...
    # Changing email doesn't deactivate the greenlight_account [FIXED].
    ...
    if @user.without_terms_acceptance { @user.update_attributes(user_params) }
      ...
      @user.without_terms_acceptance {
        ...
        user_locale(@user)
        # Updating user information will always set their password as secure [FIXED].
      }
  ...
  def update_password
    ...
    if @user.authenticate(user_params[:old_password])
      ...
      # Lack of translation [FIXED]
      @user.errors.add(:old_password, I18n.t("old_password_incorrect"))
    end

    # Notify the user that their account has been updated.
    if @user.errors.empty? && @user.without_terms_acceptance {
      @user.save && @user.update_attribute(:secure_password, true)
      # Lack of the new password state update on password change [FIXED].
    }
